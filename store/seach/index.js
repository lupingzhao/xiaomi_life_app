import api from '../../http/api.js'
export default {
	// 首页请求
	namespaced: true,
	state: {
		saechData: [],
		searchList: [],
		seachTotal: 0,
	},
	mutations: {
		setSaechData(state, value) {
			state.saechData = value
		},
		setSearchList(state, value) {
			state.searchList = value
		},
		setSeachTotal(state, value) {
			state.seachTotal = value
		},


	},
	// 异步请求
	actions: {
		// 异步操作同步化: 用一个变量接收请求的结果
		// 每个action的方法有且仅有两个参数 第一个参数是store对象 第二个参数是请求的参数
		// commit=  this.state.commit
		async getSearchWords({
			commit
		}, params) {
			try {
				let res = await api.getSearchWords()
				if (res.code === 200) {
					// console.log(res);
					// 	// 修改数据
					commit('setSaechData', res.data)
				}
			} catch (err) {
				console.log(err)
			}
		},

		async search({
			commit
		}, params) {
			try {
				let res = await api.search(params)
				// console.log(params);
				if (res.code === 200) {
					// 	// 修改数据
					commit('setSearchList', res.data)
					commit('setSeachTotal', res.total)
				}
			} catch (err) {
				console.log(err)
			}
		},



	}
}
